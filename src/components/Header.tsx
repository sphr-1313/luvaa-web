...
export default function Header() {
  ...
  const mobileMenu = mounted && open ? createPortal(
    <div
      className="lg:hidden fixed inset-0 bg-white overflow-y-auto"
      style={{ zIndex: 99999 }}
    >
      {/* Fixed header with close button */}
      <div className="fixed top-0 left-0 right-0 h-[72px] bg-white border-b border-gray-100 flex items-center justify-between px-4 sm:px-6">
      ...
      <div className="max-w-lg mx-auto px-6 py-8" style={{ paddingTop: '96px' }}>
          ...
          <div className="flex items-center justify-between h-18 py-4">
            ...
            <div className="flex items-center gap-3">
              ...
              <button
                ...
                aria-expanded={open}
                onClick={() => setOpen((s) => !s)}
                className="lg:hidden p-2.5 rounded-full bg-gray-100 hover:bg-gray-200 transition-colors relative"
                style={{ zIndex: 100000 }}
              >
                {open ? (
                  <X className="w-5 h-5 text-gray-700" />
